...
public class PinYinUtil {
		...
		private int getChsAscii(String chs) throws UnsupportedEncodingException {
			...
			if (bytes == null || bytes.length > 2 || bytes.length <= 0) { // 错误
				throw new UnsupportedEncodingException("illegal resource string");
			}
		 ...
		 * @return
		 */
		public String trans2PinYin(String str) {
			return convertAll(str);
		}
		...
		public String getFirstLetter(String str) {
			try {
				String s = convert(str.substring(0, 1)).substring(0, 1);
				if (s.compareToIgnoreCase("A") < 0
						|| s.compareToIgnoreCase("Z") > 0) {
					return "#";
				}
				return s.toUpperCase(new Locale(s));
			} catch (Exception e) {
				return "#";
			}
		}
		...
		public static void main(String[] args) {
			String str = "张三";
			String strPinYin = PinYinUtil.getInstance().convertAll(str);
			String strPinYin1 = PinYinUtil.getInstance().getFirstLetter(str);
			System.out.println(strPinYin);
			System.out.println(strPinYin1);
		}
